...
class Shipping_Method extends \WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		$this->id                 = 'boxtal_connect';
		$this->instance_id        = absint( $instance_id );
		$this->method_title       = __( 'Boxtal Connect rate', 'boxtal-connect' );
		$this->method_description = __( 'Lets you define weight/price ranges and rates for shipping and add a parcel point map.', 'boxtal-connect' );
		$this->supports           = array(
			'shipping-zones',
			'instance-settings',
		);
	...
	public function init_form_fields() {
		return array(
			'title'      => array(
				...
				'default'     => __( 'Flat rate Boxtal', 'boxtal-connect' ),
				'desc_tip'    => true,
			),
			'tax_status' => array(
				'title'   => __( 'Tax status', 'boxtal-connect' ),
				'type'    => 'select',
				'class'   => 'wc-enhanced-select',
				'default' => 'taxable',
				'options' => array(
					'taxable' => __( 'Taxable', 'boxtal-connect' ),
					'none'    => _x( 'None', 'Tax status', 'boxtal-connect' ),
				),
			),
			'rates'      => array(
				'type' => 'rates_table',
	...
	public function calculate_shipping( $package = array() ) {
		...
		WC()->session->set( 'bw_parcel_point_networks_' . $unique_identifier, null );
		...
		foreach ( $pricing_items as $pricing_item ) {
			...
			if ( WC()->session ) {
				WC()->session->set( 'bw_parcel_point_networks_' . $unique_identifier, $pricing_item['parcel_point_network'] );
			}
